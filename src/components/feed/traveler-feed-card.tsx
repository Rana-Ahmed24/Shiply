...
export function TravelerFeedCard({ listing, className }: TravelerFeedCardProps) {
  const dealsLabel =
  ...
  return (
    <Link href={listing.href} className={cn("feed-card flex flex-col", className)}>
        ...
        <div>
          {listing.rating > 0 && (
            <p className="flex items-center gap-1 text-sm font-medium text-brand-gold-light">
              <Star className="size-3.5 fill-brand-gold text-brand-gold" aria-hidden />
